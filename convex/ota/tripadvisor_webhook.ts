...
export const tripAdvisorWebhook = httpAction(async (ctx, request) => {
	if (request.method !== "POST") {
		return new Response("method not allowed", { status: 405 });
	}

	const signature = request.headers.get("x-tripadvisor-signature");
	if (!signature) {
	...
	if (integration.provider !== "tripAdvisor") {
	...
	const event = TripAdvisorClient.normalize(parsed);
	if (!event) {
		console.log(
			`[tripadvisor-webhook] ignored event type on integration ${integrationId}`,
		);
...
async function dispatchEvent(
	...
	console.log(
		`[tripadvisor-webhook] availability.update on ${integrationId} — handled by scheduled sync`,
	);
}