...
export const klookWebhook = httpAction(async (ctx, request) => {
	if (request.method !== "POST") {
		return new Response("method not allowed", { status: 405 });
	}

	const signature = request.headers.get("x-klook-signature");
	if (!signature) {
	...
	const event = KlookClient.normalize(parsed);
	if (!event) {
		console.log(
			`[klook-webhook] ignored event type on integration ${integrationId}`,
		);
...
async function dispatchEvent(
	...
	console.log(
		`[klook-webhook] availability.update on ${integrationId} — handled by scheduled sync`,
	);
}