...
export const airbnbWebhook = httpAction(async (ctx, request) => {
	...
	const timestampHeader = request.headers.get("x-airbnb-timestamp");
	...
	if (integration.provider !== "airbnb") {
	...
	const verifyResult = await AirbnbClient.verifyWebhookWithTimestamp(
		rawBody,
		signature,
		timestampHeader,
		secret,
	...
	const event = AirbnbClient.normalize(parsed);
	if (!event) {
		console.log(
			`[airbnb-webhook] ignored event type on integration ${integrationId}`,
		);
...
async function dispatchEvent(
	...
	console.log(
		`[airbnb-webhook] availability.update on ${integrationId} — handled by scheduled sync`,
	);
}