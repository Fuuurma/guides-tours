...
export const bookingWebhook = httpAction(async (ctx, request) => {
	...
	const timestampHeader = request.headers.get("x-booking-timestamp");
	...
	if (integration.provider !== "booking") {
	...
	const verifyResult = await BookingClient.verifyWebhookWithTimestamp(
		rawBody,
		signature,
		timestampHeader,
		secret,
	...
	const event = BookingClient.normalize(parsed);
	if (!event) {
		console.log(
			`[booking-webhook] ignored event type on integration ${integrationId}`,
		);
...
async function dispatchEvent(
	...
	console.log(
		`[booking-webhook] availability.update on ${integrationId} — handled by scheduled sync`,
	);
}